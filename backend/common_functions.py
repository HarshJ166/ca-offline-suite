...
def extraction_process_explicit_lines(bank, pdf_path, pdf_password, start_date, end_date, explicit_lines):
    CA_ID = "1234_temp"
    empty_idf = pd.DataFrame()
    default_name_n_num = ["_", "XXXXXXXXXX"]
...
def category_add( df):
    ...
    excel_file_path = os.path.join(BASE_DIR, "DSA_Category_sheet.xlsx")
    ...
    def categorize_bank_charges(df):
        Bank_charges = r"^(bctt|nchg|tChg|tip/scg|rate\.diff|owchquereturncharges|inwardchqreturncharge|chrg|incidentalcharges|iwchq|smschrg|chrg:sms|\*chrg:sms|nachreturncharges|fundtransfercharges|cashwithdrawalchgs|impschg)"
        df = df[
    ...
    Cash_Withdrawal = r"^(ccwd|vat|mat|nfs|atm|atm-cash-axis|atm-cash|atw|csw|atd|ati|vmt|inf|cwdr|self|cash-atm|atl/|cashpmt)"
    df.loc[
    ...
    # Update the 'Category' column for these transactions
    ...
    INB = INB[
        ~INB["Category"].str.contains("Salary Paid,Salary Received", "GST Paid")
    ]
    if not INB.empty:
        INB["Category"] = INB["Description"].apply(
            lambda x: x.split("/")[2] if "inb/" in x else x.split("/")[1]
        )
        df.update(INB)
    ...
    if not CASHDEP.empty:
        # Extract the portion of the string right after "CASHDEP" and up to (but not including) the first hyphen
        extracted_names = [
    ...
    # df.loc[(df['Description'].str.contains(last_move, regex=True)) & (df['Debit'] > 0), 'Category'] = 'Creditor'
    # df.loc[(df['Description'].str.contains(last_move, regex=True)) & (df['Credit'] > 0), 'Category'] = 'Debtor'
    ...
    df["Category"] = np.where(
        (df["Category"] == "Suspense") & (df["Credit"] > 0),
        "Payment Received",
        np.where(
            (df["Category"] == "Suspense") & (df["Debit"] > 0),
            "Payment Made",
            df["Category"],
        ),
    )
...
def summary_sheet_dsa( idf, open_bal, close_bal, eod):
    opening_bal = open_bal
    closing_bal = close_bal
